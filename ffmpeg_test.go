...
func TestFormatCommandParts(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "no audio, subtitle, or video tracks, relative path, parentheses in file name",
			inputFilePath:  "input/file.(foobar).mkv",
			audioTracks:    []int{},
			subtitleTracks: []int{},
			videoTracks:    []int{},
			expectedCmd: []string{
		...
		{
			name:           "no audio, subtitle, or video tracks, relative path, parentheses in file name, with single quote",
			inputFilePath:  "input/file.(foobar).what's.this.mkv",
			audioTracks:    []int{},
			subtitleTracks: []int{},
			videoTracks:    []int{},
			expectedCmd: []string{
				"ffmpeg",
				"-i",
				"'input/file.(foobar).what'\"'\"'s.this.mkv'",
				"-c",
				"copy",
				"'input/.tmp.file.(foobar).what'\"'\"'s.this.mkv'",
			},
		},
		{
			name:           "no audio, subtitle, or video tracks, relative path, space in filename",
			inputFilePath:  "input/file with spaces.mkv",
			audioTracks:    []int{},
			subtitleTracks: []int{},
			videoTracks:    []int{},
			expectedCmd: []string{
